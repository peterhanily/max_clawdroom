...
### Fixed
- **CI reached its first green run.** The 0.4.2 CI rework installed a swift.org toolchain but wasn't green at release: `swift-actions/setup-swift` only offers Swift ≤6.2.x, and the source's `@MainActor` annotations only type-check under the 6.3 concurrency checker. `test.yml` now installs Swift **6.3.1** directly via swiftly (the dev/ship toolchain), so CI builds and runs the full 123-test suite on every push for the first time.

## [0.4.2] — 2026-06-11

The trust-hardening release. Closes the highest-severity finding from the WWDC-week review (an unauthenticated loopback server that a visited web page could turn into a command-execution channel), ports the tvOS soul deny-list hardening to the Mac, reworks a CI pipeline that had never once gone green (diagnosis + toolchain rework shipped here; the first green run landed just after — see Unreleased), and sweeps stale public-facing docs. No on-disk format change.

### Security
- **Local OpenAI server now requires a per-install bearer token and no longer pre-approves `Bash(*)`.** `LocalOpenAIServer` (127.0.0.1:52429) shipped with no auth and a wildcard `Access-Control-Allow-Origin: *`, while each request spawned a `claude` subprocess inheriting the chat backend's `acceptEdits` + `Bash(*),Write,WebFetch(*)` defaults. Loopback is not a trust boundary against the browser: a web page the user visits can `fetch()` 127.0.0.1, so the wildcard CORS + missing auth turned any visited site into a command-execution channel with no prompt. Fixes: (1) `/v1/*` requires `Authorization: Bearer <token>`, a 40-char CSPRNG token generated per install (`Prefs.localOpenAIServerToken`), shown + copyable + regenerable in Settings; (2) CORS is granted only to validated loopback web origins — any real website gets no ACAO header, so its preflighted cross-origin POST fails; (3) the server runs a read-only tool posture (`Read,Glob,Grep`, `permissionMode: default` via `BackendSettings.defaultServerAllowedTools`) instead of inheriting the chat backend's tools — external callers can no longer run commands or edit files. Settings copy updated (the old "Loopback only — not exposed to the network" understated the browser vector). The server now restarts on the toggle / token-regenerate notification so a new token takes effect without relaunch.
- **Soul-patch deny-list now normalizes obfuscation (ported from the tvOS sibling).** `SoulPatchQueue.denyPatternMatch` was plain lowercased substring matching — `"ignore  previous"` (double space), `"1gnore previous"` (leet), `"ignore-previous-instructions"`, and `"i.g.n.o.r.e previous"` all bypassed it, despite the family-wide claim of two-pass leet-normalized matching (which existed only on tvOS). Added the two-pass matcher: precise lowercased-substring first (keeps path-shaped patterns like `.ssh/` exact), then a normalized pass (leet→letters, punctuation/whitespace→collapsed, single-letter-run glue) restricted to needles ≥6 normalized chars. 10 regression tests pin the bypass cases (suite 113 → 123).
- **`download_image` is HTTPS-only**, matching the Settings copy that already promised it (plain `http` was accepted, which also made the SSRF host check MITM-able).

### Fixed
- **CI pipeline diagnosed and reworked (not yet fully green).** Every CI run had failed at manifest parse: `Package.swift` requires swift-tools 6.2 but the hosted `macos-latest` runner ships only Xcode 16.x (Swift 6.1). The 123-test suite that underpins the v0.4.x trust positioning was producing zero signal, undetected — `docs/trust-roadmap.md` even asserted CI ran on Sonoma (corrected). `test.yml` now installs a swift.org toolchain (via swiftly) rather than relying on the runner's Xcode. Reaching a green run additionally requires Swift **6.3** — the source's `@MainActor` annotations only type-check under the 6.3 concurrency checker (the dev/ship toolchain); 6.2.1 rejects several main-actor closure captures. Wiring the 6.3 toolchain into the hosted runner is in progress.
- **SSE parser handles CRLF-framed streams and the final unterminated frame.** `OpenAIHTTPBackend`'s delegate split frames on `\n\n` only and never drained the residual buffer on completion, so an OpenAI-compatible server emitting `\r\n\r\n` produced a silently-empty reply and a stream closing right after `data: {…}\n` lost its last chunk. Now normalizes CR bytes and flushes the trailing frame. `ChannelHealth` maps 403 → unauthorized (was: only 401), matching the stream-side classifier so a 403-ing channel shows "check the API key", not "unreachable".

### Changed
- **Dependency bumps.** Sparkle 2.9.1 → 2.9.3 (the 2.9.x "Appcast Improvements" series — the appcast is load-bearing for updates). `actions/checkout` bumped off its node20 pin to v6.0.3 (node24) ahead of GitHub's 2026-06-16 node24 default.
- **Doc drift swept + gated.** README alpha banner (v0.3.1 → v0.4.1), `SECURITY.md` supported-versions table (0.3.x → 0.4.x current), and `NOTARIZATION.md` entitlements section (which still described `allow-jit` / `disable-library-validation` / `allow-dyld-environment-variables` as **present** — they were dropped with MLX in 0.1.1, so the doc overstated the attack surface). `tools/check-release.sh` now gates all three in both modes so the drift can't recur.
...
- **Memory-sanitiser tests (`EnvironmentSensorsSanitiseTests`).** 13 cases pinning `EnvironmentSensors.sanitiseForPrompt` across tag defang (open + close + case-insensitive + complete tag set), newline replacement (Unix / Windows / old-Mac), quote escape, identity / passthrough, and a realistic combined prompt-injection attempt that closes a `[memory]` block, opens an `[action]` block, and tries to escape a quoted region — all three vectors must be neutralised.
- **Streaming-parser boundary tests (`StreamingActionParserTests`).** 9 cases for `ActionParser.process(raw:from:)` with chunk-by-chunk drive that mirrors how `ChatSession` actually calls it: complete one-shot, no-action passthrough, action split at the opener boundary, byte-by-byte split mid-opener, mid-JSON split, mid-closer split, two actions with a mid-split between them, cursor monotonicity across chunks, and `[…]` prose that must NOT trigger holdback. Pins the `ambiguousOpenerSuffixLength` contract end-to-end so a future "improvement" to the streaming parser can't silently regress the chunk-boundary leak the existing implementation guards against.

Suite is now 109/109 (was 65); release manifest still consistent at v0.3.1.
