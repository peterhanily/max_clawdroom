...
The trust-hardening + CI release. Closes the highest-severity finding from the WWDC-week review (an unauthenticated loopback server that a visited web page could turn into a command-execution channel), ports the tvOS soul deny-list hardening to the Mac, fixes a CI pipeline that had never once gone green, and sweeps stale public-facing docs. No on-disk format change.
