...
### Fixed
- **CI builds for the first time.** Every CI run had failed at manifest parse: `Package.swift` requires swift-tools 6.2 but the runner's default-selected Xcode shipped Swift 6.1. The 113-test suite that underpins the v0.4.x trust positioning was producing zero signal, undetected — `docs/trust-roadmap.md` even asserted CI ran on Sonoma. `test.yml` now selects the newest installed Xcode with Swift ≥6.2 (failing loudly if none exists) before building; trust-roadmap corrected.
