...
///   - Localhost-only bind + per-install bearer token on /v1/* (see Security)
...
/// Mac can connect, AND every `/v1/*` request must carry the per-install
/// bearer token (`Prefs.localOpenAIServerToken`). Loopback alone is not
/// enough: a web page the user visits can POST to 127.0.0.1 from
/// JavaScript, so the token is what stops a cross-origin browser script
/// (or an unrelated local app) from driving the Claude backend. CORS is
/// granted only to loopback web origins; any real website gets no
/// `Access-Control-Allow-Origin`, so the browser can neither read our
/// responses nor complete a preflighted cross-origin POST.
nonisolated final class LocalOpenAIServer: @unchecked Sendable {
    ...
    /// Required bearer token for `/v1/*`. Empty ⇒ fail closed (no request
    /// authorizes), so a misconfigured server can't accidentally run open.
    ...
    init(port: UInt16 = LocalOpenAIServer.defaultPort, token: String, handler: OpenAIChatCompletions) {
        self.port = port
        self.token = token
    ...
    private func route(conn: NWConnection, method: String, path: String, headers: [String: String], body: Data) {
        ...
        // Only loopback web origins get a CORS grant (browser dev tools on
        // localhost). A real website's origin yields nil → no ACAO header,
        // so its preflight fails and it can't read our responses.
        let cors = Self.allowedCORSOrigin(headers)
        ...
        if method == "OPTIONS" {
            if let cors {
                ...
                ], corsOrigin: cors, body: "")
            } else {
                respond(on: conn, status: 403, body: "cross-origin not allowed")
            }
            return
        }

        // Bearer-token gate on the sensitive endpoints. /healthz stays open
        // so tooling can liveness-probe without the token.
        let needsAuth = (method == "POST" && routePath == "/v1/chat/completions")
            || (method == "GET" && routePath == "/v1/models")
        if needsAuth, !isAuthorized(headers) {
            respond(on: conn, status: 401, contentType: "application/json",
                    corsOrigin: cors, body: Self.errorJSON("missing or invalid bearer token"))
            return
        }

        switch (method, routePath) {
        case ("GET", "/healthz"):
            respond(on: conn, status: 200, corsOrigin: cors, body: "ok")
        case ("GET", "/v1/models"):
            handler.handleModelsList { [weak self] jsonBody in
                self?.respond(on: conn, status: 200, contentType: "application/json",
                              corsOrigin: cors, body: jsonBody)
            }
        case ("POST", "/v1/chat/completions"):
            handler.handleChatCompletions(body: body) { [weak self] event in
                ...
                case .nonStreamingJSON(let json):
                    self.respond(on: conn, status: 200, contentType: "application/json",
                                 corsOrigin: cors, body: json)
                case .startStreaming:
                    self.respondStreamingHeaders(on: conn, corsOrigin: cors)
                ...
                case .error(let status, let message):
                    self.respond(on: conn, status: status, contentType: "application/json",
                                 corsOrigin: cors, body: Self.errorJSON(message))
                }
            }
        default:
            respond(on: conn, status: 404, corsOrigin: cors, body: "not found")
        }
    }

    /// Validate `Authorization: Bearer <token>` against the configured
    /// token. Fail closed on an empty token.
    private func isAuthorized(_ headers: [String: String]) -> Bool {
        guard !token.isEmpty, let auth = headers["authorization"] else { return false }
        let parts = auth.split(separator: " ", maxSplits: 1)
        guard parts.count == 2, parts[0].lowercased() == "bearer" else { return false }
        return Self.constantTimeEquals(String(parts[1]), token)
    }

    /// Length-checked, difference-accumulating compare so the bearer check
    /// doesn't leak the token through early-exit response timing.
    ...
    /// Returns the request `Origin` only when it is a loopback web origin
    /// we're willing to echo in `Access-Control-Allow-Origin`. Any other
    /// origin (a real website) returns nil → no CORS grant.
    private static func allowedCORSOrigin(_ headers: [String: String]) -> String? {
        guard let origin = headers["origin"],
              let host = URLComponents(string: origin)?.host else { return nil }
        let loopback: Set<String> = ["localhost", "127.0.0.1", "::1"]
        return loopback.contains(host) ? origin : nil
    }
    ...
    private func respond(
        ...
        respond(on: conn, status: status, contentType: contentType, extraHeaders: extraHeaders,
                corsOrigin: corsOrigin, body: Data(body.utf8))
    }
    ...
    private func respond(
        ...
        // CORS is granted only to validated loopback origins; protocol
        // errors and non-browser callers get no ACAO header at all.
        if let corsOrigin {
            headerLines.append("Access-Control-Allow-Origin: \(corsOrigin)")
    ...
    private func respondStreamingHeaders(on conn: NWConnection, corsOrigin: String? = nil) {
        var headerLines = [
            "HTTP/1.1 200 OK",
